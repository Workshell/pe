...
namespace Workshell.PE.Content
{
    public abstract class ImportHintNameEntryBase : ISupportsLocation, ISupportsBytes
    {
        ...
        protected internal ImportHintNameEntryBase(PortableExecutableImage image, ulong offset, uint size, ushort hint, string name, bool isPadded, bool isDelayed)
        {
            _image = image;

            var calc = image.GetCalculator();
            var rva = calc.OffsetToRVA(offset);
            var va = calc.OffsetToVA(offset);
